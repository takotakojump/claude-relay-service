...
// Codex CLI 的标准客户端身份头
// 上游按客户端身份评估模型可用性与容量池，缺少这些头会被当作未知客户端处理
const CODEX_CLIENT_IDENTITY = {
...
async function handleImages(req, res) {
  ...
  try {
    ...
    const headers = {
      ...
      originator: CODEX_CLIENT_IDENTITY.originator,
      'user-agent': CODEX_CLIENT_IDENTITY.userAgent,
      version: CODEX_CLIENT_IDENTITY.version
    }
    const proxyAgent = createProxyAgent(proxy)
    const axiosConfig = {
      headers,
