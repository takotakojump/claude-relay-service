                ...
                <td class="whitespace-nowrap px-3 py-4">
                  <div v-if="account.platform === 'claude'" class="space-y-2">
                    ...
                    <div v-else class="space-y-2">
                      ...
                      <!-- 7天Opus窗口 -->
                      <div
                        v-if="hasClaudeUsageWindow(account.claudeUsage?.sevenDayOpus)"
                  ...
                  <div v-else-if="account.platform === 'openai'" class="space-y-2">
                    <div v-if="hasAnyCodexUsage(account.codexUsage)" class="space-y-2">
                      ...
                      >
                        <div class="flex items-center gap-2">
                          <span
                            class="inline-flex min-w-[32px] justify-center rounded-full px-2 py-0.5 text-[11px] font-medium"
                            :class="
                              row.isAdditional
                            ...
                            <div class="flex items-center gap-2">
                              <div class="h-2 flex-1 rounded-full bg-gray-200 dark:bg-gray-600">
                                <div
                                  :class="[
                                    'h-2 rounded-full transition-all duration-300',
                                    getCodexUsageBarClass(row.window)
                                  ]"
                        ...
                        <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                          ...
                          {{ formatCodexRemainingPercent(row.window) }}
                        </div>
                        <div class="text-[11px] text-gray-500 dark:text-gray-400">
                          重置剩余 {{ formatCodexRemaining(row.window) }}
...
const loadAccounts = async (forceReload = false) => {
  ...
  try {
    ...
    // 异步刷新 Codex 账户的额度快照（服务端带 300s 缓存，不会每次都打上游）
    if (filteredAccounts.some((acc) => acc.platform === 'openai')) {
      loadCodexUsage().catch((err) => {
        console.debug('Codex usage loading failed:', err)
      })
    }
...
const loadCodexUsage = async () => {
  const response = await httpApis.getOpenAIAccountsUsageApi()
  if (response.success && response.data) {
    const usageMap = response.data
    accounts.value = accounts.value.map((account) => {
      // 用 hasOwnProperty 而不是真值判断：null 也是有效结果（该账户确实没有额度数据）
      if (
        account.platform === 'openai' &&
        Object.prototype.hasOwnProperty.call(usageMap, account.id)
      ) {
        return { ...account, codexUsage: usageMap[account.id] }
      }
      return account
    })
  }
...
const hasClaudeUsageWindow = (window) => {
  return Boolean(
    window &&
...
const normalizeClaudeUsagePercent = (window) => {
  ...
  const hasRemainingSeconds =
    window.remainingSeconds !== null && window.remainingSeconds !== undefined
  const remainingSeconds = hasRemainingSeconds ? Number(window.remainingSeconds) : null
  const resetAtMs = window.resetsAt ? Date.parse(window.resetsAt) : null
  const resetElapsed =
  ...
  return resetElapsed ? 0 : Math.max(0, Math.min(100, utilization))
}

// 格式化 Claude 使用率百分比
const formatClaudeUsagePercent = (window) => {
  const utilization = normalizeClaudeUsagePercent(window)
  if (utilization === null) {
    return '-'
  }
...
const getClaudeUsageBarClass = (window) => {
  const util = normalizeClaudeUsagePercent(window) || 0
  if (util < 75) {
...
const formatClaudeRemaining = (window) => {
  if (
    !window ||
    window.remainingSeconds === null ||
    window.remainingSeconds === undefined ||
    Number.isNaN(Number(window.remainingSeconds))
  ) {
    return '-'
  }

  const seconds = Math.max(0, Math.floor(Number(window.remainingSeconds)))
